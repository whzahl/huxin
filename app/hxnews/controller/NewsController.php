...
// +----------------------------------------------------------------------
namespace app\hxnews\controller;
	...
	*/
    public function index(){
        ...
        if (!empty($request['type'])) {
            $where['type'] = intval($request['type']);
        }
        //时间搜索
		...
		$list = Db::name('hx_news')->where($where)->whereOr($keywordComplex)->order("id DESC")->paginate(10);
        // 获取分页显示
        $page = $list->render();
	...
	*/
	public function add(){
		if($this->request->isPost()){
			...
			$result = Db::name('hx_news')->insert($res);
			if(!empty($result)){
				$this->success("新增成功！",url('news/index'));
			}else{
				$this->error("新增失败！");
			}
		}
	...
	*/
	public function edit(){
        ...
        if ($this->request->isPost()) {
            ...
            if (Db::name('hx_news')->where(['id' => $id])->update($res) !== false) {
